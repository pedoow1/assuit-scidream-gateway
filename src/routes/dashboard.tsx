...
import { unlockAudioOnFirstInteraction } from "@/lib/sounds";
...
const QUICK: { icon: typeof BookOpen; label: string; color: string; to?: string }[] = [
  ...
  {
    icon: FlaskConical,
    label: "قسم الكيمياء الصناعية",
    color: "from-rose/50 to-gold/50",
    to: "/department",
  },
];
...
function Dashboard() {
  ...
  const currentData =
    activeTab === "auditoriums" ? AUDITORIUMS : activeTab === "classrooms" ? CLASSROOMS : LABS;
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
            {QUICK.map((q) => {
              const inner = (
                <>
                  <div
                    className={`flex h-11 w-11 items-center justify-center rounded-xl bg-gradient-to-br ${q.color} text-foreground`}
                  >
              ...
              const cls =
                "group cosmic-card flex flex-col items-center gap-2 rounded-2xl p-5 text-center transition hover:-translate-y-0.5 hover:shadow-glow";
              return q.to ? (
