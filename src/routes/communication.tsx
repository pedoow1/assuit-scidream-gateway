...
function CommunicationPage() {
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => void db.rpc("touch_presence"), 45_000);
...
function ChatView({
  ...
  // "@ mentions in this group" side button — pulled straight from the shared
  // notifications table so it always matches what the bell shows.
  ...
  async function openMention(m: AppNotification & { _message?: MessageRow }) {
    setMentionsOpen(false);
    await db.from("notifications").update({ is_read: true }).eq("id", m.id);
    setMentions((prev) => prev.map((x) => (x.id === m.id ? { ...x, is_read: true } : x)));
    if (m._message && onJumpRequested) onJumpRequested(m._message);
  }
    ...
    <div className="relative flex h-[500px] flex-col">
      <div className="mb-1.5 flex items-center justify-between">
        <button
          ...
          title="اللي عمل لي منشن"
          className="relative rounded-full p-1.5 text-foreground/50 transition hover:bg-card/60 hover:text-foreground"
        >
      ...
      {mentionsOpen && (
        <div className="absolute left-0 top-9 z-20 max-h-64 w-72 overflow-y-auto rounded-2xl border border-border/60 bg-background/95 p-2 shadow-soft backdrop-blur">
          {mentions.length === 0 ? (
            <p className="py-6 text-center text-xs text-foreground/50">محدش عملك منشن في الجروب ده</p>
          ) : (
            <div className="space-y-1">
              {mentions.map((m) => (
                <button
                  key={m.id}
                  onClick={() => void openMention(m)}
                  className={`flex w-full flex-col items-start gap-0.5 rounded-xl px-2.5 py-2 text-right text-xs transition hover:bg-card/70 ${
                    !m.is_read ? "bg-accent/10" : ""
                  }`}
                >
                  <span className="font-semibold">{m.actor_name ?? "حد"}</span>
                  <span className="truncate text-foreground/60">{m.body || "منشنك في رسالة"}</span>
                  <span className="text-[10px] text-foreground/40">{formatTime(m.created_at)}</span>
                </button>
              ))}
            </div>
          )}
...
function DMList({
  ...
  async function load() {
    setBusy(true);
    const [{ data: convRows }, { data: friendRows }] = await Promise.all([
      db
    ...
    if (friendList.length) {
      const { data: presence } = await db.rpc("get_presence", { p_ids: friendList.map((f: any) => f.id) });
