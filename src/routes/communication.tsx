    ...
    <div className="cosmic-card rounded-3xl p-4 md:p-6">
      ...
      <Tabs defaultValue="chat" dir="rtl">
        ...
        <TabsContent value="pins">
          <PinsView groupId={group.id} isAdmin={isAdmin} />
        </TabsContent>
...
// Pins
// ============================================================
function PinsView({ groupId, isAdmin }: { groupId: string; isAdmin: boolean }) {
    ...
    <div className="space-y-2 py-2">
      {pins.map((p) => (
        <div key={p.id} className="flex items-center gap-2 rounded-xl bg-card/60 px-3 py-2 text-sm">
          <span className="flex-1 truncate">{p.group_messages?.content ?? "(ميديا)"}</span>
          {isAdmin && (
            <button
              onClick={() => unpin(p.id)}
              title="إلغاء التثبيت"
