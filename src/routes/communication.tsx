...
// Shared with MessageContent's rendering regex — scans the composed text for
// "@FullName" tokens matching known members and returns the matched user
// ids, so the message can be stored with mentioned_user_ids for the DB-side
// notification trigger to pick up.
function extractMentionedIds(content: string, candidates: { id: string; full_name: string }[]): string[] {
  const named = (candidates ?? []).filter((c) => c.full_name?.trim());
  ...
  while ((match = regex.exec(content))) {
    const candidate = sorted.find((c) => c.full_name === match![1]);
  ...
  while ((match = regex.exec(content))) {
    ...
    parts.push(
      candidate ? (
        ...
        <span key={`mention-plain-${key++}`} dir="ltr">
          @{name}
        </span>
      ),
    );
...
function CommunicationPage() {
  ...
  // Presence heartbeat — lets friends' "Active" status stay accurate while
  // this tab is open, without needing a dedicated background service.
  useEffect(() => {
    if (!user) return;
    const touch = async () => {
    ...
    void touch();
    const t = setInterval(touch, 45_000);
    return () => clearInterval(t);
  }, [user]);

  // Deep-link consumption: a notification tap (mention) or a "مراسلة" button
  // on someone's profile stashes what to open in sessionStorage right before
  // navigating here, since this route has no search-param schema of its own.
  useEffect(() => {
    if (!user) return;
    try {
      const dmRaw = sessionStorage.getItem("sd_open_dm");
      if (dmRaw) {
        sessionStorage.removeItem("sd_open_dm");
        ...
        setActiveConversation({
          ...
          otherAvatarUrl: dm.otherAvatarUrl ?? null,
        });
        return;
      }
      const mentionRaw = sessionStorage.getItem("sd_open_mention");
      if (mentionRaw) {
        sessionStorage.removeItem("sd_open_mention");
        const m = JSON.parse(mentionRaw);
        if (m.groupId) void openGroupById(m.groupId);
      }
    } catch {
      /* ignore malformed/absent deep-link payloads */
    }
    ...
    <div className="cosmic-card rounded-3xl p-4 md:p-6">
      ...
      <div className="mb-4 flex items-center justify-between gap-3">
        <div className="flex min-w-0 items-center gap-3">
          <GroupAvatar name={group.name} size="lg" avatarUrl={group.avatar_url} />
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} dir="rtl">
        ...
        <TabsContent value="chat">
          <ChatView
            ...
            onJumpRequested={jumpToPinnedMessage}
          />
        </TabsContent>
...
function ChatView({
  ...
  // Lets the "@ mentions" button jump to a message the same way the Pins
  // tab does, by handing the message up to the parent's jumpTarget state.
  onJumpRequested?: (message: MessageRow) => void;
}) {
  ...
  // "@ mentions in this group" side button — only ever shows unread ones.
  // Each click jumps to the oldest unread mention and marks it read, so the
  // count ticks down and the button disappears once you've been through all
  // of them (per the requested behaviour: no dropdown, just "take me there").
  const [mentions, setMentions] = useState<(AppNotification & { _message?: MessageRow })[]>([]);
  const unreadMentions = mentions.filter((m) => !m.is_read);
  const unreadMentionsCount = unreadMentions.length;
  const [jumpingMention, setJumpingMention] = useState(false);
  ...
  async function loadMentions() {
    ...
    const { data: msgs } = msgIds.length
      ? await db.from("group_messages").select("*").in("id", msgIds)
      : { data: [] };
    const msgMap = new Map((msgs ?? []).map((m: any) => [m.id, m as MessageRow]));
    setMentions(rows.map((r) => ({ ...r, _message: r.message_id ? msgMap.get(r.message_id) : undefined })));
  }
  useEffect(() => {
    void loadMentions();
    const ch = db
      .channel(`mentions-${group.id}-${userId}`)
      .on(
        "postgres_changes",
        { event: "INSERT", schema: "public", table: "notifications", filter: `user_id=eq.${userId}` },
        (payload: any) => {
          if (payload.new.type === "mention" && payload.new.group_id === group.id) void loadMentions();
        },
      )
      .subscribe();
  ...
  async function goToNextMention() {
    const next = unreadMentions[0];
    if (!next) return;
    setJumpingMention(true);
    await db.from("notifications").update({ is_read: true }).eq("id", next.id);
    setMentions((prev) => prev.filter((x) => x.id !== next.id));
    setJumpingMention(false);
    if (next._message && onJumpRequested) onJumpRequested(next._message);
  }

  async function markMentionSeen(notificationId: string) {
    setMentions((prev) => prev.filter((x) => x.id !== notificationId));
    await db.from("notifications").update({ is_read: true }).eq("id", notificationId);
  }

  // If a mentioned message is actually sitting on screen (you're scrolled to
  // it, or it just arrived while you were already at the bottom), there's no
  // point counting it as "unseen" — clear it from the badge automatically
  // the moment it becomes visible, same as if you'd tapped the button.
  useEffect(() => {
    const root = scrollAreaRef.current;
    if (!root || unreadMentions.length === 0) return;
    const byMessageId = new Map(unreadMentions.filter((m) => m.message_id).map((m) => [m.message_id as string, m.id]));
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        for (const entry of entries) {
          ...
          const notifId = msgId ? byMessageId.get(msgId) : undefined;
          if (notifId) void markMentionSeen(notifId);
        }
      },
      { root, threshold: 0.6 },
    );
    ...
    for (const msgId of byMessageId.keys()) {
    ...
    const ch = db
      ...
      .on(
        ...
        (payload: any) => {
          ...
          const fromOthers = payload.new.sender_id !== userId;
          if (fromOthers) playMessageSound();
          if (isNearBottom()) {
            setTimeout(() => bottomRef.current?.scrollIntoView({ behavior: "smooth" }), 50);
          } else if (fromOthers) {
  ...
  async function sendText() {
    ...
    const mentionedIds = extractMentionedIds(
      body,
      Array.from(senderBadges.entries()).map(([id, b]) => ({ id, full_name: b.full_name })),
    );
    setText("");
    setReplyingTo(null);
    ...
    <div className="relative flex h-[500px] flex-col">
      <div className="mb-1.5 flex items-center justify-between">
        {unreadMentionsCount > 0 ? (
          <button
            onClick={() => void goToNextMention()}
            disabled={jumpingMention}
            title="عمل لك حد منشن — دوس تروح للرسالة"
            className="flex items-center gap-1 rounded-full bg-accent/15 px-2 py-1 text-[11px] font-semibold text-accent transition hover:bg-accent/25 disabled:opacity-60"
          >
      ...
      <div ref={scrollAreaRef} onScroll={handleScroll} className="flex-1 space-y-2.5 overflow-y-auto py-1 pr-1">
              ...
              >
                {!isSelf && (
                  <Link
                    ...
                    params={{ userId: m.sender_id }}
                    onClick={(e) => e.stopPropagation()}
                    className={`mb-0.5 block truncate text-[11px] font-bold hover:underline ${senderNameColor(senderDisplayName)} ${
      ...
      {showScrollDown && (
        <button
          ...
          title="روح لآخر الشات"
          className="absolute bottom-24 left-1/2 z-10 flex -translate-x-1/2 items-center gap-1.5 rounded-full bg-gradient-cosmic px-3 py-1.5 text-xs font-semibold text-primary-foreground shadow-rose"
        >
          <ArrowDown className="h-3.5 w-3.5" />
          {unseenCount > 0 ? `${unseenCount} رسالة جديدة` : "آخر الشات"}
        </button>
      )}
