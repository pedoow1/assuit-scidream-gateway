...
import { Loader2, ArrowLeft, UserPlus, UserCheck, UserX, Clock3, MessageCircle } from "lucide-react";
import { sendFriendNotification } from "@/hooks/use-notifications";
...
function PublicProfilePage() {
  ...
  async function respond(accept: boolean) {
    ...
    const { data: incoming, error: listError } = await db.rpc("list_incoming_friend_requests");
    if (listError) {
      setActing(false);
      return toast.error(listError.message);
    }
    const match = (incoming ?? []).find((r: any) => String(r.sender_id) === String(userId));
    if (!match) {
      setActing(false);
      toast.error("الطلب ده مش موجود دلوقتي، ممكن يكون اتلغى أو اتحل بالفعل");
      return await load();
    }
    const { error: respondError } = await db.rpc("respond_friend_request", {
      p_request_id: match.request_id,
      p_accept: accept,
    });
    if (respondError) {
      setActing(false);
      return toast.error(respondError.message);
    }
    // Reflect the new state immediately instead of waiting on load() alone —
    // this is what stopped the buttons from staying visible and inviting a
    // second click on an already-resolved request.
    ...
    setActing(false);
    toast.success(accept ? "قبلت طلب الصداقة" : "رفضت طلب الصداقة");
    await load();
  }
  ...
  async function messageThem() {
    setMessaging(true);
    const { data: gate } = await db.rpc("get_dm_gate", { p_other: userId });
    const gateRow = Array.isArray(gate) ? gate[0] : gate;
    if (gateRow?.target_privacy === "friends" && !gateRow?.is_friend) {
      setMessaging(false);
      return toast.error("الشخص ده بيسمح بالرسايل من أصحابه بس");
    }
    const { data, error } = await db.rpc("get_or_create_dm", { p_other_user: userId });
    setMessaging(false);
    if (error) {
    ...
    try {
      sessionStorage.setItem(
        ...
        JSON.stringify({
          id: data,
          otherId: userId,
          otherName: target?.full_name ?? "مستخدم",
          otherAvatarUrl: target?.avatar_url ?? null,
          nonce: Date.now(),
        }),
      );
    } catch {
        ...
        <div className="cosmic-card rounded-3xl p-8 text-center">
          ...
          {!isOwnProfile && (
            <div className="mt-6 flex flex-wrap items-center justify-center gap-2">
              ...
              <Button size="sm" variant="secondary" onClick={messageThem} disabled={messaging}>
                {messaging ? <Loader2 className="h-4 w-4 animate-spin" /> : <MessageCircle className="ml-1.5 h-4 w-4" />}
                مراسلة
              </Button>
            </div>
          )}
