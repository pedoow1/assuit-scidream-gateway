    ...
    <div className="relative min-h-screen" dir="rtl">
      ...
      <header className="relative z-10 border-b border-border/60 bg-background/80 backdrop-blur">
        <div className="mx-auto flex max-w-4xl items-center justify-between px-6 py-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <h1 className="font-display text-xl font-semibold">كل الاختبارات</h1>
          </div>
          <span />
        </div>
      </header>
      <main className="relative z-10 mx-auto max-w-4xl px-6 py-8">
        ...
        {busy ? (
          ...
          <p className="text-center text-sm text-foreground/75 py-20">لا توجد اختبارات منشورة. ادخل المادة لإنشاء اختبار.</p>
        ) : (
          <div className="grid gap-3 sm:grid-cols-2">
            {items.map((q) => (
              <Link key={q.id} to="/subjects/$subjectId/quizzes/$quizId" params={{ subjectId: q.subject_id, quizId: q.id }}
                ...
                {q.description && <p className="mt-1 text-xs text-foreground/75 line-clamp-2">{q.description}</p>}
                {q.duration_minutes && (
                  <div className="mt-3 inline-flex items-center gap-1 rounded-full border border-border px-2 py-1 text-[11px] text-foreground/75">
