...
import { Loader2, ArrowLeft, Camera, Check, X as XIcon, Lock, Volume2 } from "lucide-react";
...
function MyProfilePage() {
  ...
  const [friendsOnlyDm, setFriendsOnlyDm] = useState(false);
  ...
  useEffect(() => {
    if (profile) {
      setBio(profile.bio ?? "");
      setFriendsOnlyDm((profile as any).dm_privacy === "friends");
    }
  }, [profile]);
  ...
  async function toggleDmPrivacy(checked: boolean) {
    setFriendsOnlyDm(checked);
    setSavingPrivacy(true);
    const { error } = await db.rpc("set_dm_privacy", { p_value: checked ? "friends" : "everyone" });
    ...
    await refresh();
    toast.success(checked ? "دلوقتي بس أصحابك يقدروا يراسلوك" : "أي حد يقدر يبعتلك رسالة دلوقتي");
  }
    ...
    <div className="relative min-h-screen px-4 py-10">
      ...
      <div className="relative z-10 mx-auto max-w-xl">
        ...
        <div className="cosmic-card rounded-3xl p-8">
          ...
          <div className="mt-8 space-y-3 rounded-2xl bg-card/40 p-4">
            <h2 className="font-display text-base">الخصوصية والإشعارات</h2>
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                <Lock className="h-4 w-4 text-accent" />
                <div>
                  <p className="text-sm">رسايل من غير الأصدقاء</p>
                  <p className="text-[11px] text-foreground/50">
                    {friendsOnlyDm ? "مقفولة — بس أصحابك يقدروا يبدأوا معاك محادثة" : "متاحة — أي حد يقدر يبعتلك رسالة"}
                  </p>
                </div>
              </div>
              <Switch checked={friendsOnlyDm} onCheckedChange={toggleDmPrivacy} disabled={savingPrivacy} />
            </div>
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                <Volume2 className="h-4 w-4 text-accent" />
                <div>
                  <p className="text-sm">أصوات الرسايل والإشعارات</p>
                  <p className="text-[11px] text-foreground/50">صوت للرسايل الجديدة وصوت تاني للإشعارات</p>
                </div>
              </div>
